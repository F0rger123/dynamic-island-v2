...
internal sealed class SetupWizard : Form
{
    ...
    TabPage CheckPage()
    {
        ...
        var test = Button("Test All", (_, _) => { RefreshChecks(); status.Text = "✓ Local dependency checks refreshed. Provider tests run after setup."; }); test.Location = new Point(190, 410); p.Controls.Add(test); var next = Button("Next", (_, _) => pages.SelectedIndex = 2); next.Location = new Point(36, 410); p.Controls.Add(next); var back = Button("Back", (_, _) => pages.SelectedIndex = 0); back.Location = new Point(110, 410); p.Controls.Add(back); return p;
    }
    void RefreshChecks()
    {
